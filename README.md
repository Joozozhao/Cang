...
- **画布**：精确 **2.35:1**（消息列表封面），保存为 `<article-slug>/00-cover.png`
- **默认构图**：左侧约 55%-60% 留给大字手写主标题（≤14 字），右侧仓老师头像分身 + 一个能表达文章核心的荒诞动作；补充信息 ≤12 字
- **用色**：白底、黑色粗细混合手绘线、红色补充信息、橙色指向线；头像分身上衣固定为品牌蓝 `#002FA7`
- **参考图**：生图时必须提供 `assets/avatar-reference.jpg`；有参考封面时作为第二张构图参考图
...
5. **保存交付** → 命名：`01-topic-name.png`、`02-topic-name.png`
