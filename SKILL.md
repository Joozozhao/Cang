...
- **有生图工具**（如 Codex 的内置 `image_gen`）：每张单独生成，不要把多张图拼在一张里。
- **没有生图工具**（Claude Code 等环境默认没有）：不要假装能生成，也不要用代码画 SVG 占位图。直接按 `references/prompt-template.md` 输出每张图的**完整可粘贴提示词**，一张一段，用户拿去外部生图工具用。开头用一句话说明当前环境无法直接生成。
