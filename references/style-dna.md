...
- 线条总量：整张图的黑线加起来，接近一张便签纸随手画的量，不是一张完整插画的量。
- 主体数量：核心物件 1-2 个，配件不超过 3 个。宁可空，不要满。
- 人物占比：仓老师占画面高度约 1/3 到 1/2，不要小到看不清表情，也不要撑满画面。
- 文字总量：全图中文加起来不超过 40 个字。
- 空白：至少有一块连续的空白区域，大到能再塞下一个主体物件而不显挤。
- 颜色面积：彩色（红/橙/蓝）总面积不超过画面 10%，只用于箭头、批注、关键标记，不要用来填色块。